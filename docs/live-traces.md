...
CartoLite's Live Traces panel reuses the existing sanitized `/api/events` SSE feed. It keeps at most 1,000 accepted route or observer-only observations from the last 15 minutes. **Live** shows the newest 100 entries; **Log** shows the full retained window newest first with local date/time, normalized kind, mode, mapped hop count and route path. Pause List stops list repainting while ingestion and eviction continue; a new-observation count is shown until the list resumes.
...
Mapped route timestamps are retained in the server checkpoint for up to seven days, with a bounded sample per route. `GET /api/route-history?routes=<id,...>` returns only requested opaque route IDs and receipt times, plus `partial: true`; selecting a route in either display shows the latest retained segment timestamps beneath the Fit route and Replay actions. This is route observation history, not reconstructed packet replay or RF distance.
