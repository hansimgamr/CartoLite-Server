...
func TestPacketHistorySurvivesCheckpointWithRadio(t *testing.T) {
	...
	rssi, snr := -82.0, 6.25
	e.emitPacket(time.Now().UnixMilli(), "Text", nil, &b, &rssi, &snr)
	...
	allowed := map[string]bool{"seq": true, "id": true, "at": true, "payloadType": true, "mode": true, "observer": true, "segments": true, "rssi": true, "snr": true}
